-- ☄️ ASTEROID HUB FINAL (v2.17 – INSTANT AIMBOT AS BUTTON)
...
local Settings = {
    Aim = false,
    InstantAimbot = false,   -- toggled by button
    SilentAim = false,
    SilentAimFOV = 60,
    RapidFire = false,
...
-- Instant Aimbot as a BUTTON (toggles on/off)
CombatTab:CreateButton({
    Name = "Instant Aimbot",
    Callback = function()
        Settings.InstantAimbot = not Settings.InstantAimbot
        Rayfield:Notify({
            Title = "Instant Aimbot",
            Content = Settings.InstantAimbot and "Enabled" or "Disabled",
            Duration = 3
        })
    end
})
...
local function instantAimValidateTarget(target)
    ...
    local dist = (root.Position - head.Position).Magnitude
    if dist > Settings.FOV or dist < 5 then return false end

    if Settings.VisibilityCheck and not instantAimIsVisible(char, head) then return false end
    return true
end
...
- Click "Instant Aimbot" to toggle it ON/OFF
...
InfoTab:CreateParagraph({
    Title = "Script Log (v2.17)",
    Content = [[
✅ Universal team check (string/number safe)
✅ FOV slider (range in studs)
✅ Silent Aim with separate FOV (angle)
✅ Instant Aimbot is now a clickable button (toggles on/off)
✅ Instant Aimbot: no deadzone, re-snaps every frame
...
Rayfield:Notify({
    Title = "Asteroid Hub ☄️",
    Content = "Loaded Successfully (v2.17 – Instant Aimbot Button)",
    Duration = 5
})